...
class DrawingRobotStructure(RobotObj.RobotResearchObject):
    ...
    def convertToTDspaceList(self,input, sz):
        ...
        for each in input:
            ...
            x = c + float(lclXZero)
            y = f + float(lclYZero)
            if len(ptlst)==0 or math.sqrt(math.pow(x-ptlst[len(ptlst)-1][0],2)+math.pow(y-ptlst[len(ptlst)-1][1],2))>=0.01:#euclidean distance between prvious x and y is greater than 0.0002
